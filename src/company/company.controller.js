...
export const filterByYears = async (req, res) => {
    try {
        const { minYears, maxYears } = req.body // Obtener los años mínimos y máximos

        // Realizar la consulta para encontrar empresas dentro del rango de años
        const companies = await Company.find({ yearsCareer: { $gte: minYears, $lte: maxYears } })

        return res.send(companies) // Devolver las empresas 
    } catch (err) {
        console.error(err)
...
export const filterByOrder = async (req, res) => {
    try {
        ...
        // Realizar la consulta para encontrar empresas ordenadas 
...
export const allFilters = async (req, res) => {
    try {
        const { minYears, maxYears, category, order } = req.body;
